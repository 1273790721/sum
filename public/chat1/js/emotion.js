...
var emotion = function(targetId,editId,event){
	...
	$('#emotions').show();
	//$('#emotions').html('<div>���ڼ��أ����Ժ�...</div>');
	$('#emotions').click(function(event){
		if (event.stopPropagation){event.stopPropagation(); }//��ֹð���¼�
		event.cancelBubble = true;
	});
	for(var i=1; i<31; i++){
		$('#emotions .container').append($('<a href="javascript:void(0);"><img src="../../Public/chat/image/face/'+i+'.gif" class="face"/></a>'));
	}
...
function stripHTMLTag(text) //jsȥ��html��ǩ
{
    var reTag = /<[^img|>]+>/g;
