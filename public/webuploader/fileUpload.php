...
class FileUpload{
    ...
    //要配置的内容
    private $path = "../Upload/article/file";   //目录
    private $allowtype = array('rar','zip','doc','xls','docx','xlsx','pdf','txt','ppt');  //上传的文件类型
    private $maxsize = 99999999999;
    private $israndname = true;
    private $filepath;//lunbo
    private $originName;
    private $tmpFileName;
    private $fileType;
    private $fileSize;
...
if(isset($_POST['status'])){
    if($_POST['status'] == 'chunkCheck'){
        // 文件没有出来和这个保存的目录有关，这里就保存在根目录,修改的时候最好把源代码备份一次   做对比
    //    $target = $_POST['name'].'/'.$_POST['chunkIndex'];
