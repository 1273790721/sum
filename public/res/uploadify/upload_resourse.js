 ...
 *
 */
function uploadInit(type, dir, url, btnId, valId,public_url,va_name) {
    $("#" + btnId).uploadify({
        ...
        'fileSizeLimit': '30MB', //上传的文件大小
        'fileTypeDesc': 'mp4;rm;rmvb;wmv;avi;mkv;flv;f4v;', //上传文件类型说明
        'fileTypeExts': '*.mp4;*.rm;*.rmvb;*.wmv;*.avi;*.mkv;*.flv;*.f4v;', //上传文件类型
        'method': 'post', //提交方式
        'queueSizeLimit': 1,
        ...
        'onUploadSuccess': function(file, data, response) {  //加载上传的图片
            ...
            if(va_name){
                 $("#" + va_name).val(picjson.name);     
            }
            $("#" + valId).val(picjson.url);          
            
            $("#size").val(picjson.size);       
            $("#ext").val(picjson.ext);

            var name=$("#filename");

            if(name){
                name.val(picjson.name);
            }
