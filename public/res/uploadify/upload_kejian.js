 ...
 * url  上传调用方法的路径
 * btnId  上传按钮id
 * trainorplan  培养方案或者教学计划
 * valId  储存图片或文件id的隐藏input框id
 *
 */
function uploadInit(type, dir, url, btnId, trainorplan, valId,public_url) {
    $("#" + btnId).uploadify({
        ...
        //'cancelImg': PUBLIC + '/res/uploadify/uploadify-cancel.png',
        'auto': true, //是否自动提交
        ...
        'fileTypeDesc': 'jpg;png;jpeg;zip;doc;docx;ppt;pptx', //上传文件类型说明
        'fileTypeExts': '*.jpg;*.png;*.jpeg;*.zip;*.doc;*.docx;*.ppt;*.pptx', //上传文件类型
        'method': 'post', //提交方式
        'queueSizeLimit': 1,
        ...
        'onUploadSuccess': function(file, data, response) {  //加载上传的图片
            ...
            $("#"+trainorplan+"_size").val(picjson.size);       
            $("#"+trainorplan+"_ext").val(picjson.ext); 
            $("#mr"+trainorplan).val(''); //重新上传培养方案或教学计划时，清空默认值
 ...
 *
 */
function uploadSingleInit(type, dir, url, btnId, areaId, valId,public_url) {

    $("#" + btnId).uploadify({
        'buttonClass': "fileinput-button",
        'buttonText': "<i class='fa fa-upload'></i><span>选择文件",
        ...
        'onUploadSuccess': function(file, data, response) {  //加载上传的图片
            $("#imageli").css("margin-bottom","50px");
            var picjson = eval("(" + data + ")");
            if (picjson.status == 0) {
                alert(picjson.info);
                return false;
...
function uploadAvatarInit(type, dir, url, btnId, areaId, valId,width,height) {
    ...
    $("#" + btnId).uploadify({
        ...
        'swf': PUBLIC + '/res/uploadify/uploadify.swf', //Flash路径//getResPath
        'uploader': url, //图片提交路径
        //'uploader': '{:U("Upload/upload")}',
        'cancelImg': PUBLIC + '/res/uploadify/uploadify-cancel.png',
        'auto': true, //是否自动提交
        ...
        'onUploadSuccess': function(file, data, response) {  //加载上传的图片
            ...
            var show = "<img width='auto' height='auto' id='img_header_show' alt='Preview' src='" + PUBLIC + picjson.savepath + "thumb_" + picjson.savename + "' class='jcrop-preview'>";//getRoot
...
function uploadhtml5Init(type, dir, url, btnId, areaId, valId) {
     $("#" + btnId).uploadifive({
        ...
        'onUploadComplete': function (file, data) {
            ...
            if (picjson.type == 1) {
                var show = "<img width='100px' height='100px' alt='' src='" + PUBLIC + picjson.savepath + "thumb_" + picjson.savename + "'>";//getRoot
                var html = "<div  id='" + picjson.id + "' style='margin-left:2px;width:100px;height:130px;display:block;float:left;'>"
                        + "<div class=''>" + show + "</div>"
                        + "<button class='btn btn-sm' onclick=\"delUploadify(" + picjson.id + ",'" + valId + "')\" style='width:100px'>删除</button>"
                        + "</div>";
