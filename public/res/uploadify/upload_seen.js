function uploadInit(type, dir, url, btnId,  valId,public_url) {
    $("#" + btnId).uploadify({
        ...
        'fileTypeDesc': 'ppt;doc;docx;', //上传文件类型说明
        'fileTypeExts': '*.ppt;*.doc;*.docx', //上传文件类型
        'method': 'post', //提交方式
        'queueSizeLimit': 1,
        ...
        'onUploadSuccess': function(file, data, response) {  //加载上传的图片
            ...
            $("#" + valId).val(picjson.file);          
         
            
//            var html = "<input type='hidden' value="+picjson.ext+" id='hext' />"
//                    + "<input type='hidden' value="+picjson.size+" id='hext' />";
//
//            $('#' + areaId).append(html);
        }
    });
