...
class Rule extends Controller
{
    ...
    public function index(){
        $user = Db::connect('admin_db')->name('auth_rule')->order('id desc')->paginate(10);
	...
	public function add()
	{
		...
		if(Request::instance()->isPost()){
           ...
           // $_POST['password'] = md5($_POST['password']);
            $result = Db::connect('admin_db')->name('auth_rule')->insert($_POST);
	...
	*/
	public function edit()
	{
		if(Request::instance()->isPost()){
			//var_dump($_POST);EXIT;			
			$result = Db::connect('admin_db')->name('auth_rule')->update($_POST);
			if($result !== false){
	...
	*/
	public function del(){
         $ids = $_POST['id'];
         $model = Db::connect('admin_db')->name('auth_rule');
    ...
    public function rule(){
        $user = Db::connect('admin_db')->name('auth_group')->order('id desc')->paginate(10);
        $this->assign('users',$user);
        return $this->fetch();
    }
